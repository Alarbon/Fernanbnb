...
<%
    ...
    try {
        reservas = fernanbnbAPP.getAllReservas();
...
<section class="mt-24 ">
    <div class="text-center mb-6">
        <h1 class="text-3xl font-semibold">Datos de Todas las Reservas</h1>
    </div>
    <div class="w-full overflow-y-auto">
        <table class="w-full border-collapse table-auto">
            <thead class="bg-gradient-to-tr from-red-500 to-red-800  text-white">
            <tr>
                <th class="py-2 px-4">ID Reserva</th>
                <th class="py-2 px-4">ID Usuario</th>
                <th class="py-2 px-4">ID Propietario</th>
                <th class="py-2 px-4">ID Vivienda</th>
                <th class="py-2 px-4">Fecha Inicio</th>
                <th class="py-2 px-4">Reserva Activa</th>
                <th class="py-2 px-4">Información adiccional</th>
            ...
            <%
                if (!reservas.isEmpty()) {
                    for (Reserva r : reservas) {
                        String informacion = "-";
                        String idUsuarioReserva = "-";
                        String reservaActiva = "No";
                        v = fernanbnbAPP.buscaViviendaId(r.getIdVivienda());//Busco la vivienda de la reserva
                        ...
                        if (r.getFechaInicio().plusDays(r.getNoches()).compareTo(LocalDate.now()) > 0)
                            reservaActiva = "Sí";
                        if (r.getIdUsuario() == -1) informacion = "Es un periodo de no disponibilidad";
                        else idUsuarioReserva = String.valueOf(r.getIdUsuario());

                        if (fila % 2 == 0 || fila == 0) {%>
            <tr class="bg-gray-100">
                <td class="py-2 px-4 text-center"><%=r.getId()%>
                </td>
                <td class="py-2 px-4 text-center"><%=idUsuarioReserva%>
                </td>
                <td class="py-2 px-4 text-center"><%=p.getId()%>
                </td>
                <td class="py-2 px-4 text-center"><%=v.getId()%>
                </td>
                <td class="py-2 px-4 text-center"><%=Utils.formateaFecha(r.getFechaInicio())%>
                </td>
                <td class="py-2 px-4 text-center"><%=reservaActiva%>
                </td>
                <td class="py-2 px-4 text-center"><%=informacion%>
                </td>
