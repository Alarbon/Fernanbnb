...
<%
    ...
    try {
        ...
        usuario = (Propietario) fernanbnbAPP.actualizaPerfil(usuario);
...
<section class="container mx-auto my-8 p-4 mt-20 mb-14">
    <div class="flex flex-wrap -mx-4">
        ...
        <div class="w-full md:w-1/2 px-4 mt-4">
            <div class="w-full md:w-2/3mis px-4 mt-4 flex justify-center items-center space-x-4 ">
                <button id="addPeriodoNoDisp" class="w-48 bg-orange-500 hover:bg-orange-600 text-white px-4 py-2 rounded-md text-xl font-semibold transition duration-300">
                    Añadir Periodo
                </button>

                <button id="editarViviendaButton" class="w-48 bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-md text-xl font-semibold transition duration-300">
                    Editar Vivienda
                </button>

                <button id="eliminarViviendaButton" class="w-48 bg-gray-900 hover:bg-gray-800 text-white px-4 py-2 rounded-md text-xl font-semibold transition duration-300">
                    Eliminar Vivienda
                </button>
...
<!-- Formulario para añadir un periodo no disponible -->
<div id="periodoNoDispModal" style="display: none;" class="fixed inset-0 flex items-center justify-center z-50">
    <!-- Fondo oscuro que cubre la pantalla -->
    <div id="cerrarPeriodoNoDisp" class="fixed inset-0 bg-black opacity-60"></div>

    <div class="modal-content bg-white rounded-lg p-4 sm:p-6 md:p-8 lg:p-10 xl:p-20 z-11 relative">
        <button id="cerrarPeriodoNoDisp2"
                class="absolute top-0 right-0 transform translate-x-0 mt-2 sm:mt-4 mr-2 sm:mr-4 text-black font-bold hover:bg-gray-100 px-3 sm:px-4 py-2 rounded-full">
            X
        </button>
        <h2 class="text-xl sm:text-2xl font-semibold mb-2 sm:mb-4">Añadir Periodo No Disponible</h2>
        <p class="text-gray-400 text-sm sm:text-md mb-2 sm:mb-4">Complete el formulario para añadir un periodo no
            disponible.</p>

        <form id="periodoNoDispForm" action="establecerPeriodoNoDisponibilidad.jsp" method="get">
            <div class="mb-2 sm:mb-4">
                <label for="fechaEntradaReserva" class="block text-gray-700 font-semibold mb-2">Fecha de Entrada</label>
            ...
            <div class="mb-2 sm:mb-4">
                <label for="numNochesReserva" class="block text-gray-700 font-semibold mb-2">Número de Noches</label>
                <input type="number" id="numNochesReserva" name="numNochesReserva"
                       class="w-full px-3 sm:px-4 py-2 border rounded-md focus:outline-none focus:ring focus:border-blue-300"
                       required aria-required="true" min="1">
            </div>

            <!-- Input oculto para el idVivienda -->
            <input type="hidden" id="viviendaId" name="viviendaId" value="<%=v.getId()%>">
...
<div id="modalEditarVivienda" class="fixed inset-0 flex items-center justify-center z-50 hidden">
    <!-- Fondo oscuro que cubre la pantalla -->
    <div id="cerrarEditarVivienda" class="fixed inset-0 bg-black opacity-60"></div>

    <div class="modal-content bg-white rounded-lg p-4 sm:p-6 md:p-8 lg:p-10 xl:p-20 z-11 relative">
        <button id="cerrarEditarVivienda2"
                class="absolute top-0 right-0 transform translate-x-0 mt-2 sm:mt-4 mr-2 sm:mr-4 text-black font-bold hover:bg-gray-100 px-3 sm:px-4 py-2 rounded-full">
            X
        </button>
        <h2 class="text-xl sm:text-2xl font-semibold mb-2 sm:mb-4">Editar Vivienda</h2>
        <p class="text-gray-400 text-sm sm:text-md mb-2 sm:mb-4">Cambie los datos de la vivienda que desea modificar,
            sino deje los valores por defecto.</p>


        <!-- Formulario de Edición de Reserva -->
        <form id="formularioEditarVivienda" action="modificarVivienda.jsp" method="get">
            <!-- Campos de edición, por ejemplo: -->
            <input type="hidden" id="idVivienda" name="idVivienda" value="<%=v.getId()%>">

            <div class="mb-2 sm:mb-4">
                <label for="tituloVivienda" class="block text-gray-700 font-semibold mb-2">Nuevo Titulo </label>
                <input type="text" id="tituloVivienda" name="tituloVivienda" value="<%=v.getTitulo()%>"
                       class="w-full px-3 sm:px-4 py-2 border rounded-md focus:outline-none focus:ring focus:border-blue-300"
                       required aria-required="true">
            </div>
            <div class="mb-2 sm:mb-4">
                <label for="descripcionVivienda" class="block text-gray-700 font-semibold mb-2">Nueva
                    descripcion </label>
                <input type="text" id="descripcionVivienda" name="descripcionVivienda" value="<%=v.getDescripcion()%>"
                       class="w-full px-3 sm:px-4 py-2 border rounded-md focus:outline-none focus:ring focus:border-blue-300"
                       required aria-required="true">
            </div>
            ...
            <div class="mb-2 sm:mb-4">
                <label for="nuevoNumeroViajeros" class="block text-gray-700 font-semibold mb-2">Nuevo Número de
                    Viajeros Máximos</label>
                <input type="number" id="nuevoNumeroViajeros" name="nuevoNumeroViajeros"
                       value="<%=v.getMaxOcupantes()%>"
                       class="w-full px-3 sm:px-4 py-2 border rounded-md focus:outline-none focus:ring focus:border-blue-300"
                       required aria-required="true" min="1">
            </div>

            <div class="mb-2 sm:mb-4">
                <label for="precioNocheVivienda" class="block text-gray-700 font-semibold mb-2">Nuevo precio en
                    euros</label>
                <input type="number" id="precioNocheVivienda" name="precioNocheVivienda" value="<%=v.getPrecioNoche()%>"
                       class="w-full px-3 sm:px-4 py-2 border rounded-md focus:outline-none focus:ring focus:border-blue-300"
                       required aria-required="true" min="1">
            </div>
...
<script>
    ...
    const editarViviendaButton = document.getElementById('editarViviendaButton');

    // Obtén una referencia al modal y al botón de cierre
    const modalEditarVivienda = document.getElementById('modalEditarVivienda');
    const cerrarEditarVivienda = document.getElementById('cerrarEditarVivienda');
    const cerrarEditarVivienda2 = document.getElementById('cerrarEditarVivienda2');

    // Agrega un manejador de evento clic al botón
    editarViviendaButton.addEventListener('click', () => {
        // Abre el modal
        modalEditarVivienda.classList.remove('hidden');
    });

    // Agrega un manejador de evento clic al botón de cierre ("X")
    cerrarEditarVivienda.addEventListener('click', () => {
        // Cierra el modal
        modalEditarVivienda.classList.add('hidden');
    });
    // Agrega un manejador de evento clic fuera en el fondo oscuro
    cerrarEditarVivienda2.addEventListener('click', () => {
        // Cierra el modal
        modalEditarVivienda.classList.add('hidden');
    });

    // Agrega un manejador de evento clic al fondo oscuro
    modalEditarVivienda.addEventListener('click', (e) => {
        if (e.target === modalEditarVivienda) {
    ...
    modalEditarVivienda.querySelector('.modal-content').addEventListener('click', (e) => {
        e.stopPropagation();
    });

    const eliminarViviendaButton = document.getElementById('eliminarViviendaButton');

    eliminarViviendaButton.addEventListener('click', () => {
        // Mostrar una alerta de confirmación
        const confirmarEliminacion = confirm('¿Estás seguro de que deseas eliminar la vivienda? ');

        // Si el usuario confirma la eliminación
        if (confirmarEliminacion) {

            const idVivienda = <%=v.getId()%>;

            // Redirigir a la página JSP
            window.location.href = "eliminarVivienda.jsp?idVivienda=" + idVivienda;
        }
    });

    const addPeriodoNoDispButton = document.getElementById('addPeriodoNoDisp');

    // Obtén una referencia al modal y al botón de cierre
    const periodoNoDispModal = document.getElementById('periodoNoDispModal');
    const cerrarPeriodoNoDisp = document.getElementById('cerrarPeriodoNoDisp');
    const cerrarPeriodoNoDisp2 = document.getElementById('cerrarPeriodoNoDisp2');

    // Agrega un manejador de evento clic al botón "Añadir Periodo No Disponible"
    addPeriodoNoDispButton.addEventListener('click', () => {
        // Abre el modal
        periodoNoDispModal.style.display = 'block';
    });

    // Agrega un manejador de evento clic al botón de cierre ("X")
    cerrarPeriodoNoDisp.addEventListener('click', () => {
        // Cierra el modal
        periodoNoDispModal.style.display = 'none';
    });

    // Agrega un manejador de evento clic fuera en el fondo oscuro
    cerrarPeriodoNoDisp2.addEventListener('click', () => {
        // Cierra el modal
        periodoNoDispModal.style.display = 'none';
    });

    // Agrega un manejador de evento clic al fondo oscuro
    periodoNoDispModal.addEventListener('click', (e) => {
        if (e.target === periodoNoDispModal) {
            // Cierra el modal solo si se hizo clic en el fondo oscuro
            periodoNoDispModal.style.display = 'none';
        }
    });
    ...
    // Evita que los clics dentro del modal se propaguen al fondo oscuro
    periodoNoDispModal.querySelector('.modal-content').addEventListener('click', (e) => {
        e.stopPropagation();
    });
