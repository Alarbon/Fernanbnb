...
<%
    ...
    ArrayList<Mensaje> mensajesChat = new ArrayList<>();
    boolean notificacion = false;
    try {
        if (usuarioLogueado != null) {
            fernanbnbAPP = new FernanbnbAPP(contextPath);
            usuarioLogueado = fernanbnbAPP.actualizaPerfil(usuarioLogueado);
            if (usuarioLogueado instanceof Usuario) {
                idUsuario = String.valueOf(((Usuario) usuarioLogueado).getId());
            } else if (usuarioLogueado instanceof Propietario) {
                idUsuario = String.valueOf(((Propietario) usuarioLogueado).getId());
            } else {
        ...
        mensajesChat = fernanbnbAPP.mensajesUnSoloChat(idEmisor, idReceptor, idVivienda);
...
<div class="container mx-auto px-4 py-16 flex flex-col">
    <div class="flex-grow overflow-hidden">
        <div class="flex flex-col flex-grow p-4 space-y-4 overflow-y-auto message-container scrollbar"
             id="message-container">

            <!-- Mensajes -->
            <div class="flex flex-col p-4 space-y-4">
                <!-- Mensajes anteriores aquí -->
                <%
                    int idReceptor = 0;
                    int idEmisor = -1;
                    int idVivienda = Integer.parseInt(request.getParameter("idVivienda"));
                    for (Mensaje m : mensajesChat) {
                        int idMensaje = m.getId();
                        boolean leido = m.isLeido();
                        if (!leido) fernanbnbAPP.marcarMensajeLeido(idMensaje, Integer.parseInt(idUsuario));
                        String texto = m.getTexto();
                        String fechaEnvio = Utils.formateaFechaHora(m.getFechaEnvio());
                        String nombre = "";
                        Usuario u = fernanbnbAPP.buscaUsuariobyId(m.getIdEmisor());
                        Vivienda v = fernanbnbAPP.buscaViviendaId(idVivienda);
                        if (u != null) nombre = u.getNombre();
                        else nombre = fernanbnbAPP.buscaPropietariobyId(m.getIdEmisor()).getNombre();

                        if (Integer.parseInt(idUsuario) == m.getIdEmisor()) {
                            idEmisor = m.getIdEmisor();
                            idReceptor = m.getIdReceptor();
                            if (!m.isBorradoEmisor()) {
                %>
                <!-- Mensaje emisor -->
                <div class="flex justify-end">
                    <div
                            class="bg-blue-400 text-white rounded-bl-3xl rounded-tl-3xl rounded-tr-xl py-2 px-4 xl:max-w-10/12 sm:max-w-full">
                        <h2 class="font-semibold text-blue-950 flex justify-end ">Yo </h2>
                        <%=texto%>
                        <div class="text-sm text-gray-300 mt-1 flex justify-between items-center">
                            <span ><%=fechaEnvio%><%if (leido) {%> <span class="text-yellow-300 font-semibold ml-3">✔ Leído</span><%}%></span>
                            <button onclick="confirmDelete( <%=idMensaje%>, <%=idUsuario%>)" class="ml-4 text-gray-500">
                                Borrar
                            </button>
                        </div>

                    </div>
                    <img src="https://media.discordapp.net/attachments/1147607064525414410/1149748752081494016/perfil.png?width=342&height=340"
                         class="object-cover h-8 w-8 rounded-full ml-2" alt=""/>
                </div>
                <%
                    }
                } else {
                    idEmisor = m.getIdReceptor();
                    idReceptor = m.getIdEmisor();
                    if (!m.isBorradoReceptor()) {
                %>
                <!-- Mensaje receptor -->
                <div class="flex justify-start">
                    <img src="https://media.discordapp.net/attachments/1147607064525414410/1149748752081494016/perfil.png?width=342&height=340"
                         class="object-cover h-8 w-8 rounded-full mr-2" alt="" />
                    <div class="bg-gray-400 text-white rounded-bl-3xl rounded-tl-3xl rounded-tr-xl py-2 px-10 sm:max-w-52 max-w-1/2 overflow-x-hidden">
                        <h2 class="font-bold text-black"><%=nombre%> - <%=v.getTitulo()%></h2>
                        <%=texto%>
                        <div class="text-sm text-gray-500 mt-1 flex justify-between items-center">
                            <span><%=fechaEnvio%></span>
                            <button onclick="confirmDelete( <%=idMensaje%>, <%=idUsuario%>)" class="ml-auto text-gray-500">
                                Borrar
                            </button>
                        </div>
                    </div>
...
<!-- Input de mensaje (fuera del contenedor principal) -->
<div class="fixed bottom-16 left-0 right-0 mx-auto px-4 py-2 lg:w-3/5 sm:w-full">
    <div class="container mx-auto">
        <form id="chat-form2" action="enviarMensaje.jsp" method="get" onsubmit="return validarFormularioChat()">
            <input type="hidden" id="idEmisor" name="idEmisor"
                   value="<%=idEmisor %>">
            <input type="hidden" id="idReceptor" name="idReceptor" value="<%=idReceptor%>">
            <input type="hidden" id="idVivienda" name="idVivienda" value="<%=idVivienda%>">
            <div class="flex">
                <input id="message-input" class="flex-grow bg-gray-300 py-2 px-3 rounded-xl mr-2" type="text"
                       name="mensaje" placeholder="Escriba su mensaje aquí..." minlength="1" maxlength="250" required
                       autocomplete="off"/>
                <button type="submit" class="bg-gray-900 text-white py-2 px-4 rounded-lg">Enviar</button>
            </div>
        </form>
    </div>
</div>
<style>
    ...
    .message-container {
        ...
        /* Ajusta este valor según la altura real de tu campo de entrada de mensajes */
    }
</style>
