...
<%
    ...
    ArrayList<Propietario> usuariosPropietarios;
    ArrayList<Administrador> usuariosAdministradores;
    int fila = 0;
    try {
        usuariosNormales = fernanbnbAPP.getUsuarios();
        usuariosPropietarios = fernanbnbAPP.getPropietarios();
        usuariosAdministradores = fernanbnbAPP.getAdministradores();

    } catch (Exception e) {
        usuariosNormales = new ArrayList<>();
        usuariosPropietarios = new ArrayList<>();
...
<section class="mt-24 ">
    <div class="text-center mb-6">
        <h1 class="text-3xl font-semibold">Datos de Todos los Usuarios</h1>
    </div>
    <div class="w-full overflow-y-auto">
        <table class="w-full border-collapse table-auto">
            ...
            <tr>
                ...
                <th class="py-2 px-4">Tipo Usuario</th>
                <th class="py-2 px-4">Correo</th>
                <th class="py-2 px-4">Última conexión</th>
            ...
            <%
                if (!usuariosNormales.isEmpty() && !usuariosPropietarios.isEmpty() && !usuariosAdministradores.isEmpty()) {
                    ArrayList<Object> todosUsuarios = new ArrayList<>();
                    todosUsuarios.addAll(usuariosNormales);
                    todosUsuarios.addAll(usuariosPropietarios);
                    todosUsuarios.addAll(usuariosAdministradores);

                    for (Object user : todosUsuarios) {
                        String idUser = "";
                        String tipoUsuario = "";
                        String correo = "";
                        String ultimaConexion = "-";
                        if (user instanceof Usuario) {
                            idUser = String.valueOf(((Usuario) user).getId());
                            tipoUsuario = "Usuario";
                            correo = ((Usuario) user).getEmail();
                            ultimaConexion = cargaLastSessionWeb(((Usuario) user).getId(), contextPath);
                        } else if (user instanceof Propietario) {
                            idUser = String.valueOf(((Propietario) user).getId());
                            tipoUsuario = "Propietario";
                            correo = ((Propietario) user).getEmail();
                            ultimaConexion = cargaLastSessionWeb(((Propietario) user).getId(), contextPath);
                        } else if (user instanceof Administrador) {
                            idUser = String.valueOf(((Administrador) user).getId());
                            tipoUsuario = "Administrador";
                            correo = ((Administrador) user).getEmail();
                            ultimaConexion = cargaLastSessionWeb(((Administrador) user).getId(), contextPath);
                        }
                        if (fila % 2 == 0 || fila == 0) {%>
            <tr class="bg-gray-100">
                <td class="py-2 px-4 text-center"><%=idUser%>
                </td>
                <td class="py-2 px-4 text-center"><%=tipoUsuario%>
                </td>
                <td class="py-2 px-4 text-center"><%=correo%>
                </td>
                <td class="py-2 px-4 text-center"><%=ultimaConexion%>
                </td>
