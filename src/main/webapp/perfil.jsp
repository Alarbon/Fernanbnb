    ...
    try {
        ...
        if (usuario != null) {
            if (usuario instanceof Usuario) {
                ...
                idUsuario = ((Usuario) usuario).getId();
                nombre = ((Usuario) usuario).getNombre();
                correo = ((Usuario) usuario).getEmail();
                pass = ((Usuario) usuario).getClave();
                reservasActivas = ((Usuario) usuario).reservasPendientes().size();

            } else if (usuario instanceof Propietario) {
                usuario = fernanbnbAPP.actualizaPerfil(usuario);
                idUsuario = ((Propietario) usuario).getId();
                nombre = ((Propietario) usuario).getNombre();
                correo = ((Propietario) usuario).getEmail();
                pass = ((Propietario) usuario).getClave();
                ArrayList<Vivienda> viviendas = ((Propietario) usuario).getViviendas();
                ArrayList<Reserva> reservasPendientes;
                reservasActivas = reservasPendientes(viviendas).size();
            } else if (usuario instanceof Administrador) {
                usuario = fernanbnbAPP.actualizaPerfil(usuario);
                idUsuario = ((Administrador) usuario).getId();
                nombre = ((Administrador) usuario).getNombre();
                correo = ((Administrador) usuario).getEmail();
                pass = ((Administrador) usuario).getClave();
...
<main class="container mx-auto p-4 mt-14">
    <section id="perfil" class="bg-white p-4 sm:p-6 md:p-8 rounded-lg  text-center  mt-20">
        <!-- Información del perfil, como foto, nombre, correo y reservas activas aquí -->
        <img src="https://media.discordapp.net/attachments/1147607064525414410/1149748752081494016/perfil.png?width=342&height=3400"
             alt="Foto de perfil" class="rounded-full w-24 h-24 mx-auto mb-4">
        <h1 class="text-xl sm:text-3xl font-semibold mb-2"><%=nombre%>
        </h1>
        <p class="text-sm sm:text-base text-gray-600 mb-4">Correo Electrónico: <%=correo%>
        </p>
        <% if (usuario instanceof Propietario || usuario instanceof Usuario) {%>
        <p class="text-sm sm:text-base text-gray-600 mb-4">Reservas Activas: <%=reservasActivas%>
        <p>
                <%}%>
            <button id="editarPerfil"
                    class="block mx-auto mt-2 sm:mt-4 bg-gray-900 text-white hover:bg-gray-700 px-4 sm:px-6 py-2 rounded-md transition duration-300">
                Editar
                Perfil
            </button>
    </section>

    <section id="editarPerfilForm" style="display: none;" class="bg-white p-4 sm:p-6 md:p-8 mt-4 rounded-lg  ">
        <p class="text-xl sm:text-3xl font-semibold mb-10">Estos son sus datos actuales, puede editarlos.</p>
        <!-- Formulario para editar el perfil (nombre, correo y contraseña)  -->
        <form id="perfilForm" action="cambioPerfil.jsp" method="get">
            <div class="mb-4">
                <label for="nuevoNombre" class="block font-semibold">Nuevo Nombre:</label>
                <input type="text" id="nuevoNombre" name="nuevoNombre" value="<%=nombre%>"
                       class="w-full px-3 py-2 border rounded-md focus:outline-none focus:ring focus:border-blue-300">
            </div>

            <div class="mb-4">
                <label for="nuevoCorreo" class="block font-semibold">Nuevo Correo:</label>
                <input type="email" id="nuevoCorreo" name="nuevoCorreo" value="<%=correo%>"
                       class="w-full px-3 py-2 border rounded-md focus:outline-none focus:ring focus:border-blue-300">
            </div>

            <div class="mb-4">
                <label for="nuevaContrasena" class="block font-semibold">Nueva Contraseña:</label>
                <input type="password" id="nuevaContrasena" name="nuevaContrasena" value="<%=pass%>"
                       class="w-full px-3 py-2 border rounded-md focus:outline-none focus:ring focus:border-blue-300">
            </div>
            <div class="mb-4">
                <label for="repetirContrasena" class="block font-semibold">Repetir Contraseña:</label>
                <input type="password" id="repetirContrasena" name="repetirContrasena" value="<%=pass%>"
                       class="w-full px-3 py-2 border rounded-md focus:outline-none focus:ring focus:border-blue-300">
                <p id="error" class="text-red-500 text-sm hidden">Un error.</p>
            </div>


            <button type="submit"
                    class="bg-red-500 text-white hover:bg-red-600 px-4 sm:px-6 py-2 rounded-md transition duration-300">
                Guardar
                Cambios
            </button>
            <a href="perfil.jsp"
               class="bg-gray-900 text-white hover:bg-gray-700  px-4 sm:px-6 py-2 rounded-md transition duration-300">Volver
                a Mi Perfil</a>
