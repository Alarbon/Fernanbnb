...
<div class="container mx-auto mt-24 px-4">
    <div class="text-center mb-6">
        <h1 class="text-3xl font-semibold">Mis reservas Finalizadas</h1>
    </div>
    <div class="px- grid xl:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 gap-6 mb-16">
        <%

            ArrayList<Reserva> reservasHistorico = new ArrayList<>();

            if (usuarioLogueado instanceof Usuario) {
                reservasHistorico = ((Usuario) usuarioLogueado).reservasPendientes();
            } else {
                ArrayList<Reserva> reservasFinalizadasTemp = new ArrayList<>();
                for (Vivienda v : ((Propietario) usuarioLogueado).getViviendas()) { //Generamos el arraylist de las reservas finalizadas y pendientes
                ...
                reservasHistorico = reservasFinalizadasTemp;
            ...
            String imagen = "https://cdn.discordapp.com/attachments/1147607064525414410/1147916233560309760/h.png";
            int posReservas = 0;
            for (Reserva r : reservasHistorico) {
                Vivienda v = fernanbnbAPP.buscaViviendaId(r.getIdVivienda());
                imagen = imagenesViviendas.get(posReservas);
                posReservas++;

        %>
        <!-- Tarjeta Reserva -->
        <a href="vistaReserva.jsp?idReserva=<%=r.getId()%>&reservaFinalizada=true"
           class="bg-white rounded-lg shadow-lg overflow-hidden transition transform hover:scale-105">
            <img src="<%=imagen%>"
                 alt="Imagen de la Reserva" class="w-auto lg:h-72 md:h-52 rounded-lg object-cover">
            <div class="p-4">
                <h2 class="text-2xl font-semibold text-gray-800">Reserva en <%=v.getTitulo()%>
                </h2>
                <p class="text-black font-semibold">Fecha de llegada: <%=formateaFecha(r.getFechaInicio())%>
                </p>
                <p class="text-black font-semibold">Número de noches: <%=r.getNoches()%>
                </p>
                <p class="text-black font-semibold">Huéspedes: <%=r.getOcupantes()%>
                </p>
                <p class="text-black text-lg mt-4"><b>Total: <%=r.getPrecio() * r.getNoches()%> €</b></p>
            </div>
        </a>
